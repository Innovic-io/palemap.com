...
router.get('/', (req, res) => {
  return res.render('partials/search', { title: 'Welcome to Pale Map', city: config.city })
})
...
router.get('/place', (req, res) => {
  if (cacheService.check(req.query)) {
    ...
    return res.render('index', { title: tempQuery.name, city: config.city, placeDetails: tempQuery, placePhoto: photoUrl, placeMap: placeMapUrl })
  }
  ...
  googleService.getPlaces(url)
    .then((data) => {
      ...
      return res.render('index', { title: data.result.name, city: config.city, placeDetails: data.result, placePhoto: photoUrl, placeMap: placeMapUrl })
    })
    .catch((error) => {
      return res.status(400).json(error)
    })
